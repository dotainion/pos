...
export const Input = forwardRef(({type, title, name, className, placeholder, min}, ref) =>{
    ...
    return(
        <div className="position-relative my-4">
            ...
            <input ref={ref || inputRef} className={`form-control ${className}`} name={name} type={type} min={min} step={type === 'number' ? '0.1' : null} placeholder={placeholder || title}/>
        </div>
    )
})